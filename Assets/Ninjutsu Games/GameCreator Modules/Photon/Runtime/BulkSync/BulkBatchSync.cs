...
{
    public class BulkBatchSync : MonoBehaviour, IOnEventCallback, IPunInstantiateMagicCallback
    {
        public static BulkBatchSync Instance { get; private set; }
        public const int BatchSize = 50;
        private const byte UpdateCode = 1;
        private const byte StateUpdate = 2;

        // [Range(1, 20)]
        // [SerializeField] int m_maxSendRate = 20;

        [Range(0, 1)] [SerializeField] private float m_utilizeSendRatePercentage = 0.5f;
        ...
        [Header("Compression Infos")] [SerializeField]
        ...
        public UnitManager Manager => manager == null ? manager = GetComponent<UnitManager>() : manager;
        ...
        public static BulkBatchSync GetBatchSync()
        {
            if (!Instance)
            {
                Instance = PhotonNetwork.InstantiateRoomObject("BulkBatchSync", Vector3.zero, Quaternion.identity).GetComponent<BulkBatchSync>();
            }
            return Instance;
        }
        ...
        public void CreateBatches(List<UnitBase> units)
        {
            ...
            if (Time.time > m_timeTillNextFlush)
            {
                ...
                m_batchProcressRate = Mathf.FloorToInt(PhotonNetwork.SendRate * m_utilizeSendRatePercentage);
        ...
        private void SendBatchUpdate()
        {
            if (m_outgoingData.Count > 0)
            {
                ...
                if (PhotonNetwork.RaiseEvent(UpdateCode, byteArr, raiseEventOptions, sendOptions))
                {
                    m_sendCount++;
                }
                else
